...
// GetDeploy returns details for a specific deploy.
func (c *Client) GetDeploy(ctx context.Context, deployID string) (*Deploy, error) {
	var dpl Deploy
	if err := c.do(ctx, "GET", "/deploys/"+deployID+"/detail", nil, &dpl); err != nil {
		return nil, err
	}
	return &dpl, nil
}

// GetDeployLog returns the log for a specific deploy.
func (c *Client) GetDeployLog(ctx context.Context, deployID string) (*DeployLog, error) {
	var result DeployLog
	if err := c.do(ctx, "GET", "/deploys/"+deployID+"/log", nil, &result); err != nil {
		return nil, err
	}
	return &result, nil
}
